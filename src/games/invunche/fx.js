// EL INVUNCHE · fx.js — preallocated particle bursts + deterministic camera
// shake. No allocations after construction; bursts are event-time writes into
// fixed typed arrays, integration is a flat loop over the pool.
...
export function createFX(scene, camera) {
  ...
  // tiny deterministic hash for burst scatter (seeded per call by main's rng
  // is overkill here — visual only, never read by the sim)
  let scatterState = 12345
  function scatter() {
