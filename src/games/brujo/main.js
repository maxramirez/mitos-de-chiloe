...
/* ---------------- title narration ----------------
   The 'title' whisper belongs over the title card, not behind Begin.
   Autoplay policies usually keep a no-gesture AudioContext 'suspended',
   so this is best-effort: try at load; if blocked, the first
   pointer/key/touch starts it — before Begin. Begin never restarts a
   line already out, and still speaks it the old way if nothing managed
   to start it earlier. Silent no-op without WebAudio or the clip. */
...
function introTap() {
  ...
  if (introSpoken || state.phase !== 'title') return
  try {
