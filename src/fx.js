...
const GradeShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			float aberrPx = ( 0.8 + 2.2 * uDread ) * smoothstep( 0.15, 1.2, r );
			...
			col = mix( col, graded, uDread * 0.5 );

			// --- vignette: smoothstep falloff, base 0.35 + 0.35 * dread ---
			float vig = smoothstep( 0.45, 1.25, r );
			col *= 1.0 - ( 0.35 + 0.35 * uDread ) * vig;

			// --- dread > 0.7: slow ~1 Hz darkening pulse ---
			float pulseAmp = max( uDread - 0.7, 0.0 ) / 0.3 * 0.18;
